...
for i in range(5):
    test_data[inj_mode][inj_node][ct] = test_data[inj_mode][inj_node][ct] + \
                                              (max_min_res[0][inj_mode]-max_min_res[1][inj_mode]) / 3
    ct = ct + 1
...
plt.plot([i + td_size + config.window_size for i in range(len(consider))],
         np.array(consider) - 4, label="predict status")
plt.legend()
...
plt.plot([i + 3000 for i in range(len(consider))], np.array(consider) - 4, color="green", label="predict status")
plt.legend()
# plt.show()
