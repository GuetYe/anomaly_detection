...
with torch.no_grad():
    for i in range(nm_test_data.shape[2] - config.window_size):
        ...
        batchcat, adjcat, weicat, batchnum = combine_graph(torch.tensor(
            nm_test_data[:,:,i:i+config.window_size]).unsqueeze(0),adj,weight,True)
...
#                                               (max_min_res[0][inj_mode]-max_min_res[1][inj_mode]) / 40
#     ct = ct + 1
...
with torch.no_grad():
    for i in range(am_test_data.shape[2] - config.window_size):
        ...
        batchcat, adjcat, weicat, batchnum = combine_graph(torch.tensor(
            am_test_data[:,:,i:i+config.window_size]).unsqueeze(0),adj,weight,True)
        # print(batchcat.shape,adjcat.shape,weicat.shape,batchnum)
        out, rec, new_H, mid = model(batchcat.to(device), adjcat.to(device), weicat.to(device), H_)
        new_H = new_H.detach()
        H_ = new_H
...
plt.title("(b) anomaly injection in %s-%s timestamps of test set "
          "of modal 1 on node 45" %(inj_time,inj_time+30))
...
plt.plot([i + 3000 for i in range(len(consider))], np.array(consider) - 2, color="green", label="predict status")
plt.legend()
# plt.show()
...
plt.plot([i for i in range(mid_nm.shape[0])], mid_nm, color="blue", label="normal")
plt.plot([i for i in range(mid_am.shape[0])], mid_am, color="red", label="abnormal")
set_x_space_show()
...
plt.plot([i for i in range(rec0_nm.shape[0])], rec0_nm, color="blue", label="normal reconstruction")
plt.plot([i for i in range(rec0_am.shape[0])], rec0_am, color="red", label="abnormal reconstruction")
set_x_space_show()
