...
class PreTrainNet(nn.Module):
    ...
    def __init__(self, hidden_size, sequence_len, double_v_dim=False,
        ...
        graph_gat_output = sequence_len   // 4                 # RetNetBlock转图特征向量时的隐藏层
        # ==========================================
        ...
        # GAT获得图集特征表示
        self.gat = gnn.GATConv(block_ffns_output, graph_gat_output, heads=self.gat_head,
                            bias=True)
    ...
    def forward(self, X:torch.Tensor, A = None):
        ...
        for i in range(self.block_size):
            # 含有残差结构一层编码器
            mprint(4, f"X.shape: {X.shape}", prefix="Retnet forward")
            mprint(4, f"X.dtype: {X.dtype}", prefix="Retnet forward")
