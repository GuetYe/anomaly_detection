...
from model.mlpmodel import MLPBlock, ChannelProjection
...
class myFormer(nn.Module):
    def __init__(self, configs, adj):
        ...
        self.tre_sagcn=nn.ModuleList([spatialAttentionGCN(adj, seq_len=150, in_channels=3, out_channels=3, dropout=0.0) for _ in range(configs.e_layers)])
        ...
        self.sea_sagcn=nn.ModuleList([spatialAttentionGCN(adj, seq_len=150, in_channels=3, out_channels=3, dropout=0.0) for _ in range(configs.e_layers)])
        ...
        self.projection = ChannelProjection(configs.seq_len*2, configs.pred_len, configs.enc_in, configs.individual)
    ...
    def forward(self, x):
        ...
        for f_att,s_sagcn,s_lin in zip(self.fou_att,self.sea_sagcn,self.sea_lin):
            y = y+f_att(y)
            y = self.norm(y)
            y = y+s_sagcn(y)
            y = s_lin(y)
...
class meltFormer(nn.Module):
    def __init__(self, configs, adj):
        ...
        self.tre_sagcn=nn.ModuleList([spatialAttentionGCN(adj, seq_len=300, in_channels=3, out_channels=3, dropout=0.0) for _ in range(configs.e_layers)])
        # self.tre_project = ChannelProjection(configs.seq_len, configs.pred_len, configs.enc_in, configs.individual)
        self.tre_lin = nn.ModuleList([nn.Linear(configs.seq_len, configs.seq_len) for _ in range(configs.e_layers)])
        self.norm = nn.LayerNorm(configs.seq_len) if configs.norm else None
        # self.fou_att = nn.ModuleList([FourierBlock(fou_len=int(configs.seq_len/2)+1) for _ in range(configs.e_layers)])
        ...
        # self.cro_att=nn.ModuleList([CrossAttention(seq_len1=configs.seq_len, seq_len2=configs.seq_len, k_dim=50, v_dim=50, num_heads=5) for _ in range(configs.e_layers)])
        self.sea_sagcn=nn.ModuleList([spatialAttentionGCN(adj, seq_len=300, in_channels=3, out_channels=3, dropout=0.0) for _ in range(configs.e_layers)])
        self.sea_lin = nn.ModuleList([nn.Linear(configs.seq_len, configs.seq_len) for _ in range(configs.e_layers)])
        # self.sea_project = ChannelProjection(configs.seq_len, configs.pred_len, configs.enc_in, configs.individual)
...
if __name__ == '__main__':
    ...
    former=myFormer(args, torch.ones(size=(51, 51)).to("cuda")).to("cuda")
    # former=meltFormer(args, torch.ones(size=(51, 51)))
    # output=former(torch.randn(size=(51, 3, 300)))
    # print(output.shape)
    input=torch.randn(size=(51, 3, 300)).to("cuda")
