...
with torch.no_grad():
    ...
    for timecount in range(train_set.shape[2] - slide_win - 1 ):
        ...
        now_tensor = torch.from_numpy(train_set[:,:,timecount:timecount + slide_win]).float()
        label = torch.from_numpy(train_set[:,:, timecount + slide_win + 1]).float()
