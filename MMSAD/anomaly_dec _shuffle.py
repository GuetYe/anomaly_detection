...
log_file = open("anomaly_detection/log_hard_shuffle.txt","w",encoding="utf8")
...
inject_anomaly = inject_point[0:int(test_time*0.7)]
inject_normal = inject_point[int(test_time*0.7):test_time]
...
log_file.write("精准率:"+str(sum(target_anomaly)/ (int(test_time*0.7)-sum(target_anomaly)+sum(target_anomaly))))
log_file.write("召回率:"+str(sum(target_anomaly)/ ((test_time - int(test_time*0.7))-sum(target_normal)+sum(target_anomaly))))
